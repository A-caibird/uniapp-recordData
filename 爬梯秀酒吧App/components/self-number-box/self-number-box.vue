<template>
	<view class="u-numberbox">
		<view class="u-icon-minus" @tap.stop="$u.throttle(btnTouchStart('minus'), 1000)"  :class="{ 'u-icon-disabled': disabled || inputVal <= min }"
		    :style="{
		...
		<text class="u-number-input" :style="{
				color: color,
				fontSize: size + 'rpx',
				height: inputHeight + 'rpx',
		...
		<view class="u-icon-plus" @tap.stop="$u.throttle(btnTouchStart('plus'), 1000)" :class="{ 'u-icon-disabled': disabled || inputVal >= max }"
		    :style="{
				height: inputHeight + 'rpx',
				color: color,
				width: inputHeight + 'rpx',
	...
	export default {
		...
		methods: {
			...
			computeVal(type) {
				...
				let value = 0;
				// 减
				if (type === 'minus'&&this.inputVal-1>=this.min) {
					this.$emit('minus')
					// value = this.calcMinus(this.inputVal, this.step);
				} 
				if (type === 'plus'&&this.inputVal+1<=this.max) {
					this.$emit('plus')
...
<style lang="scss" scoped>
	...
	.u-icon-disabled {
		...
		border: 1px solid #B4B2CF !important;
		color: #B4B2CF !important;
	}
