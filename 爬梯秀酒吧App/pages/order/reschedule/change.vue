	...
	<view class="container">
		<view class="header_box">
			<u-navbar :border-bottom="false" :is-fixed="true" :background="{
				'background': '#1A1D29'
			}" title="订单改期" title-color="#FFFFFF" back-icon-color="#FFFFFF"></u-navbar>
		</view>
	...
	export default {
		...
		methods: {
			...
			async changeOrder(){
				let vm = this
				let data = {
					cardTableId: this.info.seat.id,//座位id
					arrivalTime: this.data.arrivalTime,//到店时间(yyyy-MM-dd HH:mm)
					orderId:this.orderId,
					receptionistId: this.receptorInfo.receptionistId,
					phone: this.form.phone
				};
				let type = this.info.orderType=='yaoyue'?'yao-order':'ping-order'
